...
- **R-UI-012** (accepted 2026-07-02, [[08-Decisions|D-045]]/[[08-Decisions|D-046]]): dashboard는 preview 탭을 **tmux pane에 준하는 terminal workspace**로 끌어올릴 수 있어야 한다 — ANSI/커서/스크롤백을 갖는 실제 터미널 렌더(xterm.js, [[08-Decisions|D-046]]), pane native cols×rows 반영, camp 컨텍스트를 벗어나지 않고 **자리에서 오크를 전환**(orc rail + 단축키 `[`/`]`·`Cmd/Ctrl+1..9`·퀵 스위처)하는 작업공간. 선택 SSOT는 기존 `?orc=` URL 파라미터를 유지하고(맵/rail/URL 동기화), 재현 수준은 capture-pane 한계로 명시([[08-Decisions|D-045]])하며 reduced-motion·키보드 트랩·색-비의존 상태 표시를 지킨다. 기존 P1 quick-switch/command palette(R-P1-009)·live tail(R-P1-012)을 이 workspace로 흡수·구체화한다. 런타임 화면/레이아웃 [[SPEC-203-terminal-workspace]], dock↔terminal 이관 [[SPEC-201-dashboard-screens]], 결정 [[08-Decisions|D-045]]/[[08-Decisions|D-046]].
- **R-UI-013** (accepted 2026-07-03, [[08-Decisions|D-050]]/[[08-Decisions|D-051]]): terminal workspace는 broadcast 대상 orc를 **다중 선택**(waiting만/active 전체/수동 선택)하고, 실행 전 **모든 대상을 나열한 단일 ConfirmModal**(기존 재사용, 대상별 paneId/tmuxTarget/agentType/command + broadcast input 요약)을 표시하며, 실행 후 **per-orc 결과(성공/실패 errorCode)를 집계 표시**하고, §2.9 waiting-transition 토스트의 "broadcast to all waiting" affordance와 연결할 수 있어야 한다. reduced-motion·키보드·focus-trap은 [[SPEC-202-design-accessibility]] 및 기존 ConfirmModal과 일관한다. broadcast는 새 write 경로를 만들지 않고 서버 [[SPEC-402-orchestration]](composed-input per-orc + expected 재검증)로만 나간다. UI 계약 [[SPEC-203-terminal-workspace]], 서버 [[SPEC-402-orchestration]], 결정 [[08-Decisions|D-050]]/[[08-Decisions|D-051]]. (R-CTRL-010의 UI 축이며 R-UI-012 terminal workspace의 orchestration 확장이다.)
...
- **R-CTRL-009** (accepted 2026-07-02, [[08-Decisions|D-043]]): 사용자는 명시적 **조종 모드(arm/disarm)**에서 터미널에 직접 타이핑해(keyboard passthrough) 선택된 orc에 입력을 전달할 수 있어야 한다. passthrough는 **새 write 경로를 만들지 않고** 기존 안전 write-path([[SPEC-400-control-actions]] `controlExec` single-writer, target 재검증 R-CTRL-005)로만 나가며, 확장 key allowlist·rate limit·무입력 auto-disarm을 갖고, 파괴적 chord는 confirm 게이트를 유지하고(R-CTRL-002/003 정신), audit은 arm 세션 단위로 **요약(키스트로크 원문 비저장)**한다(R-CTRL-007/R-PRIV-004 확장). 관전(Observe)에서는 키가 절대 전송되지 않아야 한다. 계약은 [[SPEC-401-interactive-input]]가 소유하고 [[SPEC-400-control-actions]]가 접점을 개정한다. 결정 [[08-Decisions|D-043]].
- **R-CTRL-010** (accepted 2026-07-03, [[08-Decisions|D-050]]/[[08-Decisions|D-051]]): 사용자는 선택한 orc 집합에 동일 입력을 **broadcast**할 수 있어야 한다. broadcast는 **새 write 경로를 만들지 않고** 각 orc에 대해 기존 안전 write-path([[SPEC-400-control-actions]] `/input` composed-input + target 재검증 R-CTRL-005)로만 나가며(arm/passthrough·multi-arm 실시간 전파 미사용), 대상 N≥2면 **모든 대상(paneId/tmuxTarget/agentType/command)을 나열한 단일 confirm**을 거치고, **per-orc 순차 실행**(single-writer 직렬화 보존)·**부분 실패 best-effort 집계**(성공/실패 per orc)·**`control.broadcast` batch audit**(원문 비저장 — targetCount/successCount/failureCount/byteLength/redactedFlag/perOrc[orcId,ok,errorCode]만)를 한다. MVP는 **단일 camp 범위**(cross-camp=forward). 계약은 [[SPEC-402-orchestration]]가 소유하고 write 경로는 [[SPEC-400-control-actions]], audit 모델은 [[SPEC-600-observability]], UI는 [[SPEC-203-terminal-workspace]]가 소유·정합한다. 결정 [[08-Decisions|D-050]]/[[08-Decisions|D-051]].
...
- **R-API-006** (accepted 2026-07-02, [[08-Decisions|D-041]]): dashboard는 focused pane 1개를 **준실시간(sub-second)**으로 갱신하는 **live pane view 채널**을 가질 수 있어야 한다 — 스캔 루프(1–5s)와 독립된 별도 채널로, 클라이언트가 `attach`/`detach`하면 서버가 해당 pane만 고빈도(가설 250–500ms) capture해 redacted 프레임을 push한다. 채널은 **read-only**([[08-Decisions|D-019]] 불변식 유지)이고 **부하 한도**(연결당 동시 attach 1, exposure on + 탭 활성 시에만 폴링, detach/종료 시 중단)를 계약으로 가진다. 프레임 스키마·프로토콜은 [[SPEC-103-pane-live-stream]], WS 프레임 카탈로그 확장은 [[SPEC-102-realtime-sync]]가 소유한다. 결정 [[08-Decisions|D-041]]. (Phase 2 `tmux -C` control mode 저지연 push는 forward.)
- **R-API-007** (accepted 2026-07-03, [[08-Decisions|D-047]]/[[08-Decisions|D-048]]/[[08-Decisions|D-049]]): dashboard는 focused pane의 live view를 **저지연(<100ms 목표)**으로 push하는 **control-mode 브리지 채널**을 opt-in으로 가질 수 있어야 한다 — `tmux -C attach` 상주 브리지가 `%output`을 **dirty-signal로만** 소비해 event-triggered capture를 트리거하고, 프레임은 여전히 기존 live view 경로([[SPEC-103-pane-live-stream]] `capture-pane` → `sanitizeCapture`/`sanitizeStyledCapture` chokepoint → 동일 `pane_view*` 프레임)로 생성한다(redaction chokepoint·wire 계약 불변, raw `%output`은 프레임/로그/디스크 미도달). 브리지는 **read-only**(`BRIDGE_COMMAND_ALLOWLIST`, `send-keys` 등 write 전면 금지·single-writer stdin fail-close)이고, 위반·crash·`%exit`·tmux 재시작 시 **종료→[[SPEC-103-pane-live-stream]] 폴링으로 silent fallback**(client 무변경, viewSeq 단조·재-attach 없음)하며, **기본은 폴링이고 브리지는 opt-in 최적화**다(어떤 브리지 장애도 정확성이 아니라 지연에만 관여). 계약은 [[SPEC-104-control-mode-bridge]], live 채널 소유는 [[SPEC-103-pane-live-stream]]. 결정 [[08-Decisions|D-047]]/[[08-Decisions|D-048]]/[[08-Decisions|D-049]].
