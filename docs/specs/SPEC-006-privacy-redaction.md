...
- 다룬 요구사항: R-PRIV-001~005, R-TMUX-001(enforcement), R-TMUX-004(safety), R-OBS-003.
...
| PF-05 | redacted preview가 browser로 전송될 때 redaction이 server-side 이전에 풀림 | Slice 2 transport | R-PRIV-002 (redaction-before-egress 불변식을 network 경계로 확장) |

> scan 슬라이스는 server/network/control 표면이 없으므로 PF-*는 **본 spec 범위가 아니다**. 후속 SPEC에서 다룰 것을 명시 표시한다.
