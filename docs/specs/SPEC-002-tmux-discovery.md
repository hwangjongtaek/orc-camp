...
decisions: [D-004, D-012, D-014, D-020, D-022]
tags:
  - specs
  - tmux
  - discovery
...
```text
#{session_name}<US>#{window_index}<US>#{pane_index}<US>#{pane_id}<US>#{pane_current_command}<US>#{pane_title}<US>#{pane_current_path}<US>#{pane_activity}<US>#{pane_pid}<US>#{pane_dead}<US>#{pane_active}<US>#{window_activity}
```
> **D-022 (tmux 버전 호환 — `pane_activity` 공백 fallback)**: 일부 tmux 빌드(관측: **tmux 3.6b**)는 `#{pane_activity}`를 **빈 문자열**로 내보낸다. 이때 `lastActivityAt`가 scan-time(`now()`)으로 붕괴해 `idle`이 영원히 발화하지 못하고 모든 pane이 "방금 활동"으로 보인다. 따라서 `FMT_P`에 `#{window_activity}`를 함께 수집해 `pane_activity`가 비면 **window 단위 activity로 fallback**한다(둘 다 비면 `now()`). window 단위라 한 window 안의 pane 간 구분은 거칠지만, working↔waiting은 [[SPEC-004-status-inference]]의 **내용 기반 신호**(S-WORK/S-IDLE-PROMPT, fingerprint diff)가 pane별로 판정하므로 정확도에 의존하지 않는다.
...
| `lastActivityAt` | string (ISO 8601) | `#{pane_activity}` → (공백이면) `#{window_activity}` | epoch seconds → ISO 8601 변환. `pane_activity`가 빈 빌드(tmux 3.6b)에서는 `window_activity`로 fallback, 둘 다 비면 `now()`(D-022). |
...
- **수집 방식(perf, 확정 구조)**: 후손 탐색은 pane마다 `ps`를 반복 호출하지 않는다. scan당 **단일 process-table snapshot 1회**(`ps -axo pid=,ppid=,etimes=,command=` 또는 OS 동등 `ps -eo pid=,ppid=,etimes=,args=`; §6 cross-platform)만 spawn하고, 그 결과를 메모리에서 ppid 링크로 walk해 각 `pane_pid`의 subtree를 구성한다. 따라서 subtree 수집의 ps spawn 수는 pane 수와 무관하게 **O(1)**이며, 종전 §2.8의 pane당 1회 `ps -p`(=O(pane) spawn)를 **대체**해 spawn 수를 오히려 줄인다(§6 Q4 latency 예산에 유리). subtree walk 자체는 메모리 내 O(process 수) 연산이다.
- **`etimes` 컬럼 추가(확정, uptime; [[08-Decisions|D-040]])**: 위 snapshot argv는 argv(`command`/`args`) **앞에** `etimes`(프로세스 시작 후 경과 **정수 초**) 컬럼을 함께 투영한다. 각 노드는 `etimeSec`로 보관된다(파싱 실패/garbled → 그 노드만 absent; throw 금지·fail-safe). 이 값은 [[SPEC-302-mascot-prestige-tiers]] §3.7 **uptime tier 폴백**(토큰 측정 불가 시 수명 proxy)의 입력이며, `etimes`는 줄 마지막의 spaces-bearing argv보다 앞에 두어 argv 파싱 경계를 깨지 않는다. **비민감**(프로세스 시작시각 — transcript/내용 아님)이므로 redaction/[[SPEC-008-usage-collection]] 게이트 밖이며, argv와 달리 `redact()`를 거치지 않고 그대로 통과한다(읽기 전용 불변식은 동일).
...
interface ProcessNode {
  ...
  etimeSec?: number;    // ps `etimes`(경과 초). 비민감(시작시각)→redact 미적용. uptime 폴백(SPEC-302 §3.7/D-040). 미가용 시 absent
}
