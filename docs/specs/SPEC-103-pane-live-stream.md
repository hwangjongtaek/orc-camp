...
const SGR_MAX = 64;                 // sgr 파라미터 문자열 최대 길이. 64는 truecolor fg+bg+attrs 조합
                                    //   ("38;2;255;255;255;48;2;255;255;255;1;3;4" ≈ 40자)까지 여유(구현 보정 2026-07-03). 유한 bound는 확정.
const SGR_RE = /^[0-9;:]{1,64}$/;   // sgr MUST match. 즉 ^[0-9;:]{1,SGR_MAX}$ (SGR 숫자 파라미터 only, ESC/문자 불가)
...
- **`PaneViewSession`(런타임 구성체 정의, 확정)**: 한 attach를 소유하는 **per-connection 런타임 객체**다. 정체성 = `(connection, orcId, attach-epoch)`; attach 성립 시 생성돼 `pane_view_seed`(viewSeq=0)를 발행하고 `pane_view_end` 시 소멸한다(연결당 활성 ≤1, §2.2/§3.3). 이 세션은 (a) `viewSeq` 증가의 **유일 소유자**이고 (b) 해당 pane에 대해 **capture scheduler를 정확히 하나** 가진다(§3.1/§3.8 at-most-one-pending). capture tick의 **트리거 소스**(기본 interval 폴링, 또는 Phase 2 브리지의 `%output` event — [[SPEC-104-control-mode-bridge]])가 무엇이든 세션은 **동일 객체로 유지**되므로, 트리거 소스 전환은 `pane_view_seed` 재전송·재-attach 없이 `viewSeq` 단조성을 보존한다(§6 Q6). 본 정의가 SSOT이며 [[SPEC-104-control-mode-bridge]]가 handoff·scheduler 계약에서 이를 참조한다.
...
- **트리거 소스 불변 노트(2026-07-03, [[08-Decisions|D-047]]/[[08-Decisions|D-049]])**: capture tick의 **트리거는 interval-polled(기본) 또는 Phase 2 브리지의 `%output` event-triggered([[SPEC-104-control-mode-bridge]]) 둘 중 하나**일 수 있다 — 어느 소스든 그 뒤의 §2.5 capture→sanitize→프레임 생성·`viewSeq` 증가는 **본 spec 계약이 불변**이다. 브리지는 프레임을 만들지 않고 tick을 **언제** 돌릴지만 정한다(HYBRID). 트리거 소스 전환은 같은 `PaneViewSession` 위에서 일어나 seed 재전송·재-attach가 없다(§6 Q6).
...
SPEC-103-AC-18 (R-PRIV-008 / R-UI-012 / R-API-006 / [[08-Decisions|D-042]])  [styled 프레임 wire 형식 — no raw escape·sgr charset·정렬]
  ...
  Then (a) spans 는 lines 에 병렬(spans.length === lines.length)이고 각 StyleSpan 은
       ...
       (e) 모든 span 의 sgr 이 정규식 ^[0-9;:]{1,SGR_MAX}$(SGR_MAX=64)를 만족한다
...
- **Q1 — `PANE_VIEW_INTERVAL_MS`·`T`·failure 임계·tmux 부하**: 250–500ms 폴링 주기, per-call timeout `T`(≈2000ms)와 interval의 관계(§3.8 silent degradation), `MAX_VIEW_CAPTURE_FAILURES`(가설 3), pane당 최대 안전 Hz, 고빈도 `capture-pane`이 tmux 서버에 주는 실측 부하는 [[SPEC-007-test-validation]] 하니스로 측정해 확정한다(§3.1/§3.7/§3.8). 부하가 크면 주기를 늘리거나 changed-tail만 emit(§2.3)로 완화. **1차 실측(2026-07-02, 실 tmux 3.6b, 30줄 pane, `list-panes`+`capture-pane` 왕복)**: capture당 **p50≈10ms / p95≈36ms** — 250ms interval 대비 ~1/25로 백로그 여유 충분(단일 focused pane, MAX_ATTACH=1 기준). **styled(`-e` + tokenize + style-remap) 실측(2026-07-03, 120×40 색 출력 pane)**: capture당 **p50≈17ms / p95≈32ms**로 plain 대비 소폭 상향이나 여전히 250ms interval 대비 여유 충분(§6 Q2). 다중 pane·대용량 scrollback 부하는 [[SPEC-007-test-validation]] 정식 하니스로 후속 확정.
- **Q2 — changed-tail vs full-window emit (RESOLVED 2026-07-03 — full-window 유지)**: `pane_view`가 매 tick 전체 window를 보낼지, 직전 대비 변경 tail만 보낼지(대역폭 vs 렌더 정확도)는 측정으로 결정한다. 어느 쪽이든 redacted 버퍼 상에서만 diff한다(§3.4). **1차 실측(2026-07-03, 실 tmux 3.6b, 120×40 색 출력 pane, styled `-e` + tokenize + style-remap 포함)**: full-window 프레임(`{lines, spans}` JSON) **p50≈1.6 KB / p95≈2.5 KB**, capture당 **p50≈17ms / p95≈32ms**, tick-to-tick changed-line ratio≈2.6%. localhost(127.0.0.1, MAX_ATTACH=1)에서 4Hz 기준 ≈6–10 KB/s로 대역폭이 **비이슈**이며, changed-tail diff는 (i) redacted-버퍼 diff + `spans` 재정렬 복잡성과 (ii) seed/gap 회복 로직을 추가하는 데 비해 이득이 없다. **disposition: full-window 유지(확정, MVP)**. 재검토는 P2 multi-client/remote(대역폭이 실제 제약이 되는 경우, Q7)로 forward한다.
...
- **Q5 — Phase 1.5 styled 도입 시점 (RESOLVED 2026-07-03 — 형식 확정, 게이트는 SPEC-007 코퍼스 green 대기)**: styled 프레임의 **wire 형식은 §2.3.1로 확정**됐다 — `lines`(redacted plain)는 불변, 선택적 `spans: StyleSpan[][]`(`{start,end,sgr}`, UTF-16 반열림 offset, 정렬·비중첩, `sgr` MUST match `^[0-9;:]{1,SGR_MAX}$` with SGR_MAX=64)를 병렬 오버레이로 추가하며 raw escape byte는 wire에 실리지 않는다(AC-18~21). **unlock predicate**도 §3.4-3으로 확정: (a)[[SPEC-006-privacy-redaction]] §2.8 approved(충족) · (b)[[SPEC-007-test-validation]] `TC-M-STYLED` secret-recall==1.0==plain · (c)runtime self-check(styled redacted plain == plain 산출). 잔여는 (b) styled 코퍼스가 CI에서 green이 되는 것뿐이며, 그전까지 §3.4-2 plain fallback 고정(AC-06). 색 재현의 **저지연 개선**(control mode)은 Q6 forward.
- **Q6 — Phase 2 control-mode 확장 (RESOLVED 2026-07-03 — [[SPEC-104-control-mode-bridge]]가 소유)**: 저지연(<100ms) push는 `tmux -C attach` 상주 브리지로 개선한다([[08-Decisions|D-041]] (c)). 그 브리지는 tmux 바이너리를 상주 attach하는 **새 subprocess 진입점**이라 `tmuxExec` allowlist 밖이므로, 브리지가 발행하는 명령을 **read-only sub-allowlist**(attach·refresh-client·subscribe류, `send-keys` 금지)로 별도 고정하는 sub-계약이 필요하다. **(2026-07-03 확정: 이 예시 목록은 [[08-Decisions|D-048]]에서 effective allowlist=∅로 대체됐다 — `refresh-client -C`는 실제 window resize(SIGWINCH)를 유발해 read-only를 깨고 `capture-pane`은 redaction chokepoint를 우회하므로 둘 다 금지; 브리지는 명령을 발행하지 않고 `%output` notification만 읽는다.)** **해소**: 그 sub-계약을 **[[SPEC-104-control-mode-bridge]]**(ID 확정, [[08-Decisions|D-047]]/[[08-Decisions|D-048]]/[[08-Decisions|D-049]] Proposed)가 소유한다. 핵심 정합: (a) 브리지는 **본 spec의 동일 프레임**(`pane_view_seed`/`pane_view`/`pane_view_end`)을 발행한다 — 새 프레임·새 wire 계약을 만들지 않고 `%output`을 dirty-signal로만 써 **본 spec §2.5 capture 경로를 트리거**할 뿐이다(HYBRID, [[08-Decisions|D-047]]). (b) fallback = **트리거 소스 swap** — 브리지와 interval 폴링이 **동일 `PaneViewSession`**을 먹이므로, 브리지 장애 시 폴링으로의 전환은 `pane_view_seed` 재전송이 없고(**재-attach 없음**) `viewSeq`가 전환을 가로질러 **단조 +1을 유지**한다([[SPEC-104-control-mode-bridge]] §2.6). (c) 기본 트리거는 **폴링(본 spec)**이고 브리지는 opt-in이며 어떤 브리지 장애도 client-visible 프레임을 바꾸지 않는다([[08-Decisions|D-049]]). 본 spec은 Phase 1(폴링) 계약을 확정하고 브리지 상세는 [[SPEC-104-control-mode-bridge]]에 위임한다.
