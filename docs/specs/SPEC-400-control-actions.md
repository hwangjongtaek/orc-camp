...
본 spec이 정의하는 것: (a) read-only vs write-path 경계(§2.1), (b) 3개 control endpoint의 request/response·error(§2.2~§2.3·§2.9), (c) `send-keys` 인자 템플릿과 injection 차단(§2.3), (d) key allowlist(§2.4), (e) 안전 게이트 파이프라인(§2.5), (f) 실행 직전 target 재검증(§2.6, R-CTRL-005), (g) control context·confirm(§2.7, R-CTRL-006·R-CTRL-003), (h) audit event producer 계약과 privacy(§2.8, R-CTRL-007), (i) inspector → action → modal → execute → toast UI flow(§2.11), (j) **키보드 passthrough 접점(§2.12, R-CTRL-009)** — passthrough의 계약 본체는 [[SPEC-401-interactive-input]]가 소유하고 본 spec은 write 경로·allowlist·audit·rate limit 재사용 경계만 명시한다.

> **2026-07-02 개정 note([[08-Decisions|D-043]])**: Terminal Workspace 키보드 passthrough([[SPEC-401-interactive-input]], R-CTRL-009)를 위해 **§2.12(passthrough 접점)와 AC-18/AC-19를 추가**한다. 개정은 append-only다 — 기존 §2.1~§2.11·AC-01~AC-17·불변식(single-writer·R-CTRL-008·R-CTRL-005)은 **변경 없이 유지**된다. passthrough는 **새 writer를 만들지 않고** 본 spec의 `controlExec`·`/input`·`/key` egress를 재사용하며, base `KEY_ALLOWLIST`를 느슨하게 하지 않는다(§2.12). 이에 따라 `status`를 `approved→draft`로 되돌리고 `decisions`에 `D-043`, `requirements`에 `R-CTRL-009`(접점)를 추가했다.
...
- **keyboard passthrough 접점**(§2.12, R-CTRL-009, 2026-07-02 개정): passthrough가 재사용하는 write 경로(`controlExec`·`/input`·`/key`)·allowlist superset 관계·audit/rate limit 재사용 **경계**. 계약 본체(arm/disarm·auto-disarm·확장 allowlist·batch audit)는 [[SPEC-401-interactive-input]] 소유.
- 다룬 요구사항: **R-CTRL-001~008**, R-CTRL-009(passthrough 접점), R-UI-004(control entry → action flow 측면).
...
- **`text`**: UTF-8 string, `byteLength ≤ MAX_INPUT_BYTES`(가설 4 KiB, §3). **control byte(`U+0000–U+001F` 및 `U+007F`) 포함 시 `422 control_char_not_allowed`로 거부**(§2.3.1, 리뷰 반영). `submit`(기본 `true`)이면 literal text 전송 후 별도 `Enter` key 전송(§2.3).
...
| --- | --- | --- | --- |
| `literal` | `['send-keys','-t',paneId,'-l','--',text]` | `text`를 **리터럴 키스트로크**로 pane에 입력 | `-l`=리터럴(키 이름 해석 안 함) + `--`=옵션 파싱 종료. `text`가 `C-c`/`-X`/`$(...)`여도 **문자 그대로** 입력됨. shell:false로 셸 미해석. **단, `-l`은 키 이름 해석만 막고 raw control byte는 그대로 전달**하므로 §2.3.1 control-byte 필터를 반드시 선행한다. |
...
#### 2.3.1 literal control-byte 필터 (확정, 2026-07-02 리뷰 반영 — R-CTRL-002/003, [[08-Decisions|D-006]])

**보안 핵심**: `-l`은 tmux의 **키 이름 해석**만 막고, `text`에 담긴 **raw control byte는 그대로 pane에 전달**한다. 즉 `text`가 byte `0x03`(`^C`)·`0x04`(`^D`)·`0x1a`(`^Z`)·`0x1c`(`^\`)를 포함하면 `-l --`를 통과해도 pane 안 프로그램은 이를 **SIGINT/EOF/SIGTSTP/SIGQUIT 상당의 제어 문자**로 받는다. 이는 §2.4 destructive-chord 제외와 `/interrupt` confirm modal(§2.7, R-CTRL-003)을 **우회**하는 경로다.

- **확정 규칙**: `literal` 템플릿으로 보내는 `text`는 `controlExec` 호출 **직전**(schema Gate 2) **control byte를 포함하면 `422 validation_error`(code `control_char_not_allowed`)로 거부**한다 — 금지 집합은 `U+0000–U+001F`(모든 C0 control) **및** `U+007F`(DEL)다. **strip이 아니라 reject**를 채택한다(부분 전송으로 인한 예측 불가 상태 방지).
  - **예외 없음**: 개행(`\n`/`0x0a`)·탭(`0x09`)·복귀(`0x0d`)도 control byte이므로 literal `text`에서 거부한다. 줄바꿈/제출은 `/key`(`Enter`)로, 탭은 `/key`(`Tab`)로, 모든 제어 기능은 `/key` allowlist(§2.4) 또는 `/interrupt`(§2.7)로만 전달한다.
- **결과(확정)**: destructive 제어 기능은 **오직** (a) §2.4 `KEY_ALLOWLIST`(파괴적 chord 제외), (b) `/interrupt` confirm 이중 게이트(§2.7)로만 도달 가능하다. literal 경로로는 제어 문자를 넣을 수 없으므로 "destructive chord는 confirm 게이트를 거친다"는 보장이 **literal 경로에서도** 성립한다(AC-20).
- **passthrough 계승(확정)**: [[SPEC-401-interactive-input]]의 passthrough literal egress도 이 필터를 그대로 상속한다(§2.12). 측정·adversarial 케이스는 [[SPEC-007-test-validation]](control-byte injection 케이스)로 검증한다.
...
| 2 | **method/schema** | `422 validation_error`(unknown 필드·타입·길이·`confirmed`·key 형식) / `422 control_char_not_allowed`(literal `text`의 control byte, §2.3.1) | R-CTRL-008(자유 명령 필드 부재), R-CTRL-002/003 |
...
| `422` | `control_char_not_allowed` | literal `text`에 control byte(`U+0000–U+001F`/`U+007F`) 포함(§2.3.1) | 2 | aborted(인증 통과분) |
...
### 2.12 keyboard passthrough 접점 (R-CTRL-009, [[08-Decisions|D-043]]) — 2026-07-02 개정

키보드 passthrough의 **계약 본체**(arm/disarm 수명주기·auto-disarm·확장 allowlist·keystroke rate cap·배치 audit·재검증 재적용 timing)는 [[SPEC-401-interactive-input]]가 소유한다. 본 절은 passthrough가 **본 spec의 무엇을 어떻게 재사용/확장하는지의 경계**만 고정한다. 아래 어느 것도 기존 §2.1~§2.11 불변식을 느슨하게 하지 않는다.

- **egress = controlExec/endpoint 재사용(확정)**: passthrough의 tmux egress는 **본 spec §2.1 `controlExec` single-writer**(고정 `send-keys` 3 템플릿)와 **기존 endpoint `/input`(literal)·`/key`**를 그대로 쓴다. [[SPEC-401-interactive-input]]는 **두 번째 writer나 새 egress endpoint를 만들지 않는다**. printable 문자는 `literal` 템플릿(`-l --`, `submit:false`), named 키는 `key` 템플릿으로 나간다. passthrough literal도 **§2.3.1 control-byte 필터를 상속**해 raw 제어 byte가 destructive 경로를 우회할 수 없다(리뷰 반영). `/interrupt`는 passthrough에 노출되지 않는다(§2.7 destructive 경로 불변).
- **request 스키마 확장(확정, R-CTRL-008 유지)**: `InputRequest`/`KeyRequest`에 optional `passthrough?: { armSessionId: string }` **마커**를 추가한다(`InterruptRequest`에는 추가하지 않는다). 이 마커는 **유일 필드가 `armSessionId`**이며 **자유 명령/subcommand/args 필드가 아니다** — §2.2의 strict schema(unknown 필드 거부)와 R-CTRL-008(구조적 임의 명령 불가)은 그대로 성립한다. 마커가 있으면 [[SPEC-401-interactive-input]] 규칙(arm-session·확장 allowlist·rate cap·배치 audit)이 적용되고, 없으면 기존 form-path 그대로다.
- **allowlist 확장 경계(확정)**: [[SPEC-401-interactive-input]]의 `INTERACTIVE_KEY_ALLOWLIST`는 본 spec §2.4 `KEY_ALLOWLIST`의 **superset**이며 **armed passthrough(마커 있는 `/key`)에서만** 쓰인다 — 관계식 `KEY_ALLOWLIST ⊂ INTERACTIVE_KEY_ALLOWLIST`. **마커 없는 form-path `/key`는 여전히 base `KEY_ALLOWLIST`만** 허용한다(§2.4 무변경). 두 allowlist 모두 파괴적 chord(`C-c`/`C-d`/`C-z`/`C-\\`)를 **제외**하며, `C-c`는 passthrough로 열리지 않고 §2.7 confirm modal + `/interrupt {confirmed:true}` 이중 게이트로만 전송된다.
- **audit producer 확장(확정, R-CTRL-007)**: form-path의 개별 action은 종전대로 **per-action `control.result`**(§2.8)를 산출한다. passthrough egress는 **per-action `control.result`를 발행하지 않고**, [[SPEC-401-interactive-input]] §2.9가 **arm-session 단위 배치 요약 1건**(`ActivityEvent` code `control.passthrough_session`)으로 산출한다 — 키스트로크 원문·literal 텍스트·전송 key 시퀀스·token은 §2.8 privacy 불변식대로 **어떤 필드에도 직렬화하지 않는다**([[SPEC-006-privacy-redaction]] §2.5, [[SPEC-600-observability]] §2.7). 두 producer 모두 [[SPEC-600-observability]] canonical `ActivityEvent`로 매핑된다([[08-Decisions|D-028]]).
...
```text
SPEC-400-AC-18 (R-CTRL-009, R-CTRL-008, [[08-Decisions|D-043]])  [passthrough single-writer 확장]
  Given passthrough(arm/disarm + armed egress)와 scan 을 함께 실행하는 동안
  When 실제로 spawn된 tmux argv 전체와 request 스키마를 관측하면
  Then passthrough 로 send-keys 를 발행한 호출은 모두 controlExec 의 literal/key 템플릿과 일치하고,
       [[SPEC-401-interactive-input]] 의 /passthrough/arm·/disarm 은 send-keys 를 전혀 발행하지 않으며,
       passthrough 마커는 armSessionId 외 자유 명령 필드를 추가하지 않아(unknown 필드 422 거부)
       controlExec single-writer 와 R-CTRL-008 이 passthrough 까지 확장 유지된다.
```

```text
SPEC-400-AC-19 (R-CTRL-009, R-CTRL-002/007, [[08-Decisions|D-043]])  [allowlist/audit 경계]
  Given passthrough 마커가 있는 /key 와 없는 form-path /key 를 각각 보낼 때
  When allowlist 적용과 audit 산출을 검사하면
  Then 마커 있는 요청은 INTERACTIVE_KEY_ALLOWLIST(=KEY_ALLOWLIST superset)로 검증되고 마커 없는 요청은
       base KEY_ALLOWLIST(§2.4) 로만 검증되며(둘 다 파괴적 chord 제외),
       form-path action 은 per-action control.result 를, passthrough egress 는 per-action event 없이
       세션 종료 시 control.passthrough_session 배치 event 1건([[SPEC-401-interactive-input]] §2.9)을 산출한다.
```

```text
SPEC-400-AC-20 (R-CTRL-002, R-CTRL-003, [[08-Decisions|D-006]])  [literal control-byte 필터 — 2026-07-02 리뷰 반영]
  Given /input 의 text 가 raw control byte(예: 0x03=^C, 0x04=^D, 0x1a=^Z, 0x1c=^\, 0x0a=개행) 를 포함할 때
  When 그 요청을 처리하면
  Then controlExec 호출 전에 422 control_char_not_allowed 로 거부되어 send-keys 가 전혀 발행되지 않고,
       -l -- 를 통해서도 어떤 제어 문자도 pane 에 도달하지 못하며,
       모든 제어 기능은 /key allowlist(§2.4, 파괴적 chord 제외) 또는 /interrupt confirm 게이트(§2.7)로만 전달된다
       (destructive-chord/confirm 우회 경로 차단; [[SPEC-007-test-validation]] control-byte injection 케이스).
```
...
| 요구사항 | 다루는 방식 | 검증 AC |
| --- | --- | --- |
| R-CTRL-001 | `/input` literal `-l --` 전송 + Enter, 단일 paneId target, control-byte 필터(§2.3.1) | SPEC-400-AC-01, SPEC-400-AC-02, SPEC-400-AC-20 |
| R-CTRL-002 | `/key` KEY_ALLOWLIST 강제, 비허용 키 거부, 제어 기능은 key/interrupt로만(§2.3.1) | SPEC-400-AC-03, SPEC-400-AC-20 |
| R-CTRL-003 | `/interrupt` UI modal + backend `confirmed:true` 이중 게이트, `C-c`; literal control-byte로 confirm 우회 차단(§2.3.1) | SPEC-400-AC-04, SPEC-400-AC-11, SPEC-400-AC-20 |
...
| R-CTRL-009 (접점) | passthrough가 controlExec/`/input`·`/key` egress·per-pane 직렬화·§2.6 재검증 재사용, `INTERACTIVE_KEY_ALLOWLIST` superset 경계, 배치 audit 접점(본체는 [[SPEC-401-interactive-input]]) | SPEC-400-AC-18, SPEC-400-AC-19 |
...
| [[08-Decisions\|D-043]] (passthrough) | 새 writer 없음·controlExec 재사용·allowlist superset·batch audit 경계(§2.12) | SPEC-400-AC-18, SPEC-400-AC-19 |

> R-CTRL-004는 token gate **메커니즘**을 [[SPEC-100-server-lifecycle]]이 1차 소유하고 본 spec이 **control endpoint 적용**을 소유한다(R-SEC-003 floor 재사용). R-UI-004의 inspector **레이아웃**은 [[SPEC-201-dashboard-screens]], destructive **a11y**는 [[SPEC-202-design-accessibility]]가 소유한다. **R-CTRL-009**(passthrough)의 **1차 소유는 [[SPEC-401-interactive-input]]**이며, 본 spec은 write 경로·allowlist·audit·rate limit **재사용 경계(§2.12)**만 소유한다. 전체 추적 매트릭스 통합은 [[SPEC-900-traceability-rollup]].
