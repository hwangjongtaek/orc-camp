...
> **판정: CONDITIONAL GO.** 세션 로그에서 **집계 스칼라만** 추출하는 usage collector의 구현을 **조건부 승인**한다. 아래 G1~G8 게이트를 **전부** 충족하는 구현만 허용되며, "금지" 목록 중 **하나라도** 위반하면 즉시 **NO-GO**(머지 차단)다. 게이트는 각 수용 기준(§6)으로 테스트 가능하다.
...
| G7 | identity 상관(correlation)이 **모호하면 null** — 다른 orc로 misattribute 금지 | SPEC-008-AC-08 |
...
- **디스크 전수 스캔 금지·deterministic locate**: pane→세션 파일은 다음 순서로 **좁게** 찾는다.
  1) pane의 process argv(이미 redaction 경계를 통과한 [[SPEC-002-tmux-discovery]] §2.9 `processTree`)에 **명시 session-id**(예 `--session-id`/`--resume <id>`)가 있으면 그 id로 파일을 직접 지목.
  2) 없으면 orc `cwd`(이미 수집됨)로 provider별 디렉터리를 **1단계만** 결정(Claude: `cwd`를 encoded-dir 규칙으로 변환해 그 **한 디렉터리** 안의 `*.jsonl`만 나열). 디렉터리 안에서 **명백히 단일·최근**(mtime이 pane `lastActivityAt`와 정합) 세션만 채택.
  3) 위로 확정 불가(후보 0 또는 복수 모호) → `usage=null`(§4.3).
  - `$HOME`/디스크 전역 glob·walk·`find`는 **금지**. 항상 고정 provider root 아래의 **단일 디렉터리 나열**만 허용한다.
...
- usage는 `paneId` 권위로 특정 orc에 귀속한다(D-017). 상관 체인은 §4.2 locate 순서(명시 session-id → cwd+단일최근 → null)와 동일하다.
- **모호하면 null**: 한 디렉터리에 복수 세션이 있고 상관 신호가 없으면(또는 cwd↔encoded-dir 정합이 깨지면) **추측하지 않고 `usage=null`**. 임의 선택은 T-U06 misattribution이므로 금지.
...
- **codex**: `~/.codex/sessions/` 하위 rollout JSONL에서 usage/token_count 류 payload의 숫자만 합산. 형식 미확정이면(Q1) `null` 반환(추측 금지). 형제 secret 파일 접근 금지(§4.2).
...
SPEC-008-AC-07 (R-PRIV-007)  [absent/unreadable/ambiguous → null]
  Given (a) 세션 파일 부재, (b) 권한 거부/parse 실패, (c) 한 디렉터리에 상관 신호 없는 복수 후보일 때
  When collector가 실행되면
  Then 세 경우 모두 usage=null을 반환하고(추측·단정 없음),
       (c)에서 임의의 한 세션을 선택하지 않는다.
```
...
| --- | --- | --- |
| R-PRIV-007(신규 proposed, 1차 소유) | 세션 로그 surface의 data-minimization·file-access 경계·비저장·degradable·identity 계약 | SPEC-008-AC-01·03·04·05·06·07·08·09·10·11 |
...
| [[08-Decisions|D-039]](신규) | data-minimization chokepoint·root confinement·GO 조건 | §1.1, SPEC-008-AC-01~11 |
| [[08-Decisions|D-016]](확장) | redaction chokepoint를 본 surface로 확장(defense-in-depth) | SPEC-008-AC-02 |
| [[08-Decisions|D-017]](재사용) | usage 귀속의 권위는 `paneId` | SPEC-008-AC-08 |
...
- **Q1 — Codex 로그 형식/위치 미확정**: `~/.codex/sessions/<Y>/<M>/<D>/rollout-*.jsonl`의 usage payload 키가 실측 미확정. 확정 전 Codex provider는 `null` 반환(추측 금지). **검토 필요.**
- **Q2 — 세션↔pane 상관 신뢰도**: 명시 session-id(argv) 부재 시 cwd+mtime 휴리스틱의 false-attribution 위험(T-U06). [[SPEC-007-test-validation]]에서 측정·보정. non-leak 다음으로 큰 correctness 리스크. **검토 필요.**
