...
- ⚠️ **신규 위협면(forward 착수 시 security-privacy-engineer 사전 리뷰 필요)**: transcript/usage 읽기는 tmux capture를 넘어 **세션 로그 파일을 직접 읽는 새 read surface**다(민감 내용 포함 가능). 본 spec은 *숫자 집계(token/cost)만* 소비하고 원문은 비저장이어야 하나, 파일 접근 경로·권한·redaction 경계는 [[SPEC-006-privacy-redaction]] threat model 확장으로 별도 검토한다. 현재는 forward로 격리돼 blocker는 아니다.
