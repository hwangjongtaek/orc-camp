...
- ⚠️ **신규 위협면 — 보안 리뷰 완료, CONDITIONAL GO**: transcript/usage 읽기는 tmux capture를 넘어 **세션 로그 파일을 직접 읽는 새 read surface**다(민감 내용 포함 가능). security-privacy-engineer 사전 리뷰가 이를 [[SPEC-008-usage-collection]](privacy/security 소유, [[08-Decisions|D-039]])로 격리하고 **CONDITIONAL GO**를 판정했다. 구현은 SPEC-008 §1.1 GO 조건(G1~**G9**: 집계 스칼라만 산출·content 구조적 skip·비저장·root confinement·bounded read·degradable null·misattribution 금지(mtime 추측 금지)·provider-pluggable·**open-handle(fd) 상관 최소권한**)을 **전부** 충족할 때만 착수할 수 있고, transcript 원문/경로/secret을 저장·log·직렬화·반환하는 어떤 경로도 **금지(NO-GO)**다. (amended 2026-06-30: 세션↔orc 상관은 mtime 추측이 아니라 agent 프로세스의 **실제 open fd**로 결정적 수행 — SPEC-008 §4.2a; Codex는 schema 실측 확인 전까지 `null`.) 본 spec은 그 계약이 산출한 `OrcUsage`(숫자 집계)만 소비한다(원문 비접촉). 현재는 forward로 격리돼 blocker는 아니다.
- `orc-unknown`(agent type 미확정): usage 출처가 더 약할 수 있다(어떤 agent인지 모름). 획득 불가 시 `usage=null`→tier 0이 정상 동작이며, 단정하지 않는다.
- `usage=null` 또는 두 축 모두 `null` ⇒ **§3.7 uptime 폴백**(`Orc.uptimeSec`); 그것도 `null`이면 tier 0.
...
```ts
const PRESTIGE_TIER1_MIN_TOKENS = 100_000;   // tuned 2026-06-30 (D-036): 관측 per-session usage 정렬
const PRESTIGE_TIER2_MIN_TOKENS = 500_000;
const PRESTIGE_TIER3_MIN_TOKENS = 2_000_000;
// 보조 축(tokens 분해 불가 시), ~$30/1M-token 정렬
const PRESTIGE_TIER1_MIN_COST_USD = 3;
const PRESTIGE_TIER2_MIN_COST_USD = 15;
const PRESTIGE_TIER3_MIN_COST_USD = 60;
```
...
### 3.7 uptime 폴백 axis (토큰 측정 불가 시 — D-040)

토큰 상관이 실패하는 orc(`usage=null` 또는 `cumulativeTokens=null`)가 대다수다(라이브 3/28만 상관 — 복수 세션 파일·misattribution 금지). 그래서 tier 판정을 **다중 신호 우선순위 폴백**으로 둔다:

1. `usage.cumulativeTokens != null` → §3.2 token 임계.
2. else `usage.cumulativeCostUsd != null` → §3.2 cost 임계.
3. **else `orc.uptimeSec != null` → uptime 임계(아래)**.
4. else 0.

- **입력**: `Orc.uptimeSec: number | null` — pane subtree의 **agent 런타임 프로세스 경과시간(초)**. [[SPEC-002-tmux-discovery]] ps 스냅샷에 `etimes` 컬럼을 더해 산출한다. **비민감**(프로세스 시작시각 — transcript 아님)이라 [[SPEC-008-usage-collection]] 게이트 무관. 측정 불가(프로세스 부재=terminated / platform 미지원) → `null`.
- **임계 상수(SSOT, 초; 튜닝 대상)**: `PRESTIGE_TIER1_MIN_UPTIME_S = 86_400`(1d) / `TIER2 = 259_200`(3d) / `TIER3 = 604_800`(7d). 경계 inclusive(≥). manifest `prestige`에 character별 override는 두지 않는다(공통). **DAY 스케일 튜닝(2026-06-30)**: 라이브 세션이 수일~수주 지속해 1h/4h/12h 시드는 alive orc 거의 전원을 T3로 포화시켰다(uptime은 세션 *수명*이라 idle 장수 세션도 최상위가 됨) → 일 단위로 상향.
- **의미(주의)**: uptime은 *강도*가 아닌 **수명(longevity)** proxy(idle 포함)다 → 토큰보다 약한 근거이므로 **항상 토큰 다음**(토큰 있으면 uptime 무시). tier가 신호 혼합 composite가 되는 것은 §3.x 표시에서 **근거(basis: tokens|cost|uptime)**를 함께 노출해 보완한다.
- **불변**: 판정된 raw tier는 신호 종류와 무관하게 §3.2 latch(복합 키)·§3.2 게이트(prestige 블록 유무)·§3.3 하향 폴백·§3.4 합성을 그대로 거친다. terminated orc는 uptime=null이나, 토큰 로그가 남아 있으면 토큰으로 커버된다.
- **결정성**: `rawTierForUsage(usage, uptimeSec, thresholds)`는 순수 함수다(같은 입력 → 같은 raw tier). SPEC-300 §2.4 결정성은 `uptimeSec`를 입력에 포함시켜 성립.

## 4. Acceptance Criteria

- **SPEC-302-AC-01** (R-P2-008) — tier 판정 임계
  - Given tiered character, `usage.cumulativeTokens` = 0 / 150k / 800k / 3M fixture에서
  - When tier를 판정하면
  - Then 각각 `rawTier` = 0 / 1 / 2 / 3 이다. 경계값(정확히 100,000 / 500,000 / 2,000,000)은 해당 tier로 **올림**(≥)이다.
- **SPEC-302-AC-02** (R-P2-008) — cost 보조 축·경계·null
  - Given `cumulativeTokens=null` & `cumulativeCostUsd` = 2.99 / 3 / 20 / 60 / `usage=null` fixture에서
  - Then `rawTier` = 0 / 1 / 2 / 3 / 0 이다(tokens 부재 시 cost 축, cost 경계 $3/$15/$60은 inclusive(≥), 둘 다 부재 시 0).
...
- **SPEC-302-AC-09** (R-P2-008, R-P1-004) — 비-mascot tiered character의 tier resolution + 합성(일반화 증명)
  - Given **resolved characterKey를 직접 주입**: `orc-claude-storm-shaman`(tier 2 variant `available`, animation 미보유), `usage.cumulativeTokens=800k`(→tier 2), `status=active`, reduced-motion 아님; 그리고 동일 조건의 `orc-codex-field-engineer`·`orc-unknown`·`orc-iron-commander` fixture에서
...
- **SPEC-302-AC-14** (R-P2-008, D-040) — uptime 폴백 + precedence
  - Given tiered character, `usage.cumulativeTokens=null` & `cumulativeCostUsd=null`, `uptimeSec` = 86399 / 86400 / 259200 / 604800 / null fixture에서
  - Then `rawTier` = 0 / 1 / 2 / 3 / 0 이다(uptime 경계 86400/259200/604800초 = 1d/3d/7d inclusive; uptimeSec=null이면 0).
  - And `cumulativeTokens != null`인 fixture에서는 uptime이 더 높아도 **token 축이 우선**한다(precedence — uptime 무시).
