...
- **측정 스냅샷 (2026-06-27, 현재 데이터셋·엔진)**:
  ...
  - **M2 live (status, `scripts/measure-status-live.mts` + 통제 e2e `TC-E-WAITING`)**: 실 환경 6 cycle — 거의 모든 orc가 weak `active`(LOW 0.40), **over-active 0**(변화 없음+오래된 활동인데 active로 단정한 케이스 0 → 엔진 보수적·정직). live snapshot에서 status 변별력이 약한 건 `pane_activity`(TUI redraw)가 항상 "최근 활동"으로 떠 S-RECENT 약신호가 살아있기 때문(LOW confidence로 정직 표현). **waiting**: 통제된 정적 `(y/n)` prompt pane(2 cycle prior)에서 엔진이 정확히 `waiting`(≥0.5)로 분류 — `TC-E-WAITING` 통과 → **waiting 로직은 현실적 prompt에서 정상 작동**. live snapshot의 외견상 "0% waiting"은 광범위 probe가 상시 입력 박스(`>`)를 prompt로 과집계한 **측정 artifact**이지 엔진 결함이 아니다. 결론: detection과 달리 status는 **코드 수정 불요**(measure→verify→결함 없음). 정밀 waiting recall은 실 Claude Code 권한 UI 라벨링이 잔여.
