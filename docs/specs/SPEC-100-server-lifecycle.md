...
- **control-mode bridge 진단(정보 전용 — [[SPEC-104-control-mode-bridge]] §6 Q3 종결, [[08-Decisions|D-053]])**: [[SPEC-600-observability]] `DoctorDiagnostics.bridge` block(`{ enabled, tmuxVersion, controlModeSupported, socketArgs }`)은 control-mode 브리지의 **정적 capability + 설정값(`liveViewBridge`)**만 표기하는 **비-exit advisory**다 — 위 5개 check와 무관하게 exit code(§3.5)에 **기여하지 않는다**. `controlModeSupported`는 `tmux -V` 파싱 + `ignore-size`(≥3.2) 기준의 정적 판정이고, 브리지는 opt-in 지연 최적화라 미지원이 doctor fail이 아니다([[08-Decisions|D-049]]). 값·redaction·런타임 fallback 이력 경계는 [[SPEC-600-observability]] §2.9 소유(현재 트리거 소스·fallback 이력은 activity audit 소관이라 doctor 범위 밖). doctor는 read-only `tmux -V`만 추가 실행한다.
