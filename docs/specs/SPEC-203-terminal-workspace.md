...
- **terminal 모드는 map layout과 독립·full-width 고정(확정)**: terminal 모드는 map layout(`ui.layoutMode` `full`/`split`/`dock`, [[SPEC-201-dashboard-screens]] map dock 배치)과 **독립이며 항상 full-width로 렌더링**한다 — terminal workspace에는 `.oc-map`/`.oc-dock` 2-열 grid를 적용하지 않는다. layout 스위처는 **map 모드에서만 노출·적용**되며, persisted `layoutMode`는 terminal 진입으로 **변경되지 않는다**(map 복귀 시 이전 `split`/`dock`이 그대로 복원된다). 즉 컨테이너의 표시 layout은 terminal 모드에서 `full`로 강제되고 persisted 선택은 map 모드에서만 반영된다.
...
- **뷰포트 높이 사용자 조절(확정)**: Terminal Viewport의 높이는 **사용자가 조절 가능**하며(관전 영역이 고정·과소했던 문제 해소), 뷰포트와 status bar 사이의 **resize 핸들**을 드래그하거나(포인터) 핸들 focus 시 **키보드**(`ArrowUp`/`ArrowDown` 스텝, `Home`/`End` 최소/최대)로 변경한다. 높이는 client UI 선호로 **영속화**(`ui.terminalViewportHeight`, localStorage)하고 **하한/상한으로 clamp**(가설 상수 `TERMINAL_HEIGHT_MIN`/`MAX`, §6)한다. 핸들은 `role="separator"`+`aria-orientation="horizontal"`+`aria-value*`로 **키보드 완결·색-비의존**(불변식 ④, [[SPEC-202-design-accessibility]] §2.4)이다. 높이 변경은 §2.2 zero-layout-shift를 깨지 않으며 cols×rows는 프레임 값 그대로다(물리 fit만 변함).
- **flicker-free 원자적 redraw(확정)**: capture 기반 프레임은 매 갱신마다 화면 전체를 재구성한다. 이 재그리기는 **단일 원자적 write**(클리어 시퀀스를 write 스트림 안에 포함)로 수행해야 하며 — 동기 clear→비동기 write 분리(예: `reset()` 후 별도 `write()`)로 인해 그 사이 **중간 blank 프레임이 페인트되는 것을 금지**한다(프레임마다 깜빡임의 원인). 클리어(scrollback+화면+home)는 styled lines·cursor와 **한 번의 write로 함께 파싱**되며, 가능하면 synchronized-update(`DEC 2026`) 래핑으로 감싼다(미지원 단말은 무해하게 무시). **무변화 프레임은 재그리기를 생략**하고(직전 payload와 동일하면 write 자체를 skip), grid 크기(`cols`×`rows`)가 실제로 변할 때만 resize한다.
