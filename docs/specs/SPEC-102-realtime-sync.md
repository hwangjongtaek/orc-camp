...
3. `seq`는 연결 내 frame 누락·중복을 감지하는 보조키다. `seq`가 비연속(서버 측 frame drop)이면 client는 안전하게 resync(§3.5)한다.
