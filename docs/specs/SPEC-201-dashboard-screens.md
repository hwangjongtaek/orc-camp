...
| R-PRIV-006 | TerminalPreview 노출 토글 + line-count 컨트롤(UI·behavior), backend redacted text only, 별도 Preview 탭 lazy mount(노출면 최소화) | SPEC-201-AC-09, AC-10, AC-16 |
...
- **Q6 — Preview 탭의 interactive(SSH-control) 전환(향후 작업)**: §2.5 TODO대로 Preview를 read-only redacted tail에서 ssh/tmux control-mode/PTY 브리지 기반 **대화형 터미널**(키 입력·scrollback·resize)로 확장하는 것은 별도 작업이다. 이는 (a) read-only 불변식(D-019)·redaction 불변식(불변식 ③, R-PRIV-002)을 재검토하고, (b) control write-path([[SPEC-400-control-actions]])·새 PTY/ws 전송 계약·privacy([[SPEC-006-privacy-redaction]]) 게이트를 동반해야 하므로 **본 MVP slice out-of-scope**(P1+). 채택 시 별도 spec/요구사항 승격이 필요하다. **검토 필요.**
