...
- **배치 모델(window = lane)**: orc를 `windowIndex`로 그룹핑해 lane(camp area)으로 묶고, lane 내부는 `paneIndex` 오름차순으로 slot에 배치한다([[SPEC-005-data-contract]] §3.4 정렬과 정합). lane 헤더에 window 식별을 표기한다.
...
SPEC-201-AC-03 (R-UI-003)
  ...
  Then orc 는 windowIndex 로 lane 그룹핑되고 lane 내부는 paneIndex 오름차순으로 배치되며,
       각 slot 은 raw tmuxTarget 라벨과 status badge(label)를 가지고,
