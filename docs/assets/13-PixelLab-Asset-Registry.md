...
10. **Character prestige tier (delivered 5종 × T1~T3 = 15 variant) — mascot 3 variant 생성·승인 완료(2026-06-29), 나머지 4종 planned**: 누적 token/cost 기반 외형 단계(각 archetype 정체성 유지하며 갑옷/의장·장비·`active` 외형 점층 강화). 각 character의 base에서 `create_character_state`로 tier당 1 variant 생성(8방향 rotation; animation은 후속 단계). 설계·turnkey prompt·manifest schema는 [[15-Character-State-Model]] §2/§4, 런타임 계약은 [[SPEC-302-mascot-prestige-tiers]], 결정 [[08-Decisions|D-036]]. **진행 상황(2026-06-29)**: 인증 복구 → `orc-high-warchief-mascot` 3 tier 생성·owner 승인(IP 리뷰 통과)·manifest `prestige` 반영 완료(아래 ▼ 기록). 나머지 4종은 동일 절차로 순차 생성 예정. **`orc-iron-commander` 포함**(pool roaming skin으로 배정된 orc의 usage로 tier; control 상징 역할은 별개 축).
    - 대상·source base id·suffix:
      ...
      - `orc-unknown` ← `50519f63-0f39-49da-9782-f75b93b8152c` → `-feltouched`/`-felreaver`/`-felascendant` (**전면 개정 2026-06-30: fel demon-hunter 컨셉**, T1부터 fel 전투검; IP: Illidan-class 발산 — §2.7 blocking)
      - `orc-iron-commander` ← `b9a2dbff-d392-4703-9555-3ac2e1cf8df2` → `-enforcer`/`-marshal`/`-sovereign`
    - 진행 시 채울 항목(character·tier별): `pixellab_character_id`, `group_id`(base와 공유), export zip + SHA-256, extracted root `sprites/<character>/tiers/<suffix>/`, 사용 `edit_description`·`seed`·`use_color_palette_from_reference`, 8방향 rotation 완료/드리프트 QA 결과.
    - ▼ **생성 기록 — `orc-high-warchief-mascot`(2026-06-29 생성·승인)**. group_id `a1e04745-4ed7-44ec-8eb9-261f2c2da302`(base와 공유). 각 tier는 base `77acab26-…`에서 직접 `create_character_state`(체이닝 없음), seed=default(미지정), 8방향 rotation만(animation은 후속). SHA-256 = 8 rotation(south,east,north,west,SE,NE,NW,SW) 순서 concat 후 `shasum -a 256`. QA: 8방향 정체성/체형 유지, 무기(도끼) **8방향 grip 일관**(north-east는 base와 동일하게 cape 가림 — 결함 아님), text artifact 없음, single axe. **1차 T2/T3은 무기 부유·후면 누락·text artifact 결함으로 폐기**(삭제) 후 [[15-Character-State-Model]] §4.1 무기 일관성 정정 prompt로 재생성한 것이 아래 ID다.
      | tier | suffix | `pixellab_character_id` | root | `use_color_palette_from_reference` | IP 리뷰 | SHA-256(8-rot) |
      | --- | --- | --- | --- | --- | --- | --- |
      | 1 Ironclad Veteran | `-veteran` | `e1e3b502-38bc-49d9-ac7f-3a4259ff26d3` | `sprites/orc-high-warchief-mascot/tiers/veteran/Orc_High_Warchief_Mascot_Veteran` | true | n/a(저티어) | `1a65bf3be7ca4f105e2af281c5810c947c963cf16a8d674da948829e19ff8cfc` |
      | 2 Ember Champion | `-champion` | `55edad33-8b53-4a4d-a2af-0a9e75f503ee` | `…/tiers/champion/Orc_High_Warchief_Mascot_Champion` | false | passed(owner, 2026-06-30) | `384c3b1fbb8eb0568218fd634c50e8642fb91935f8f62bf1b6531d30f00fcdf4` |
      | 3 Legendary Warlord | `-warlord` | `c748be94-f87a-4b68-a4f1-c7207cfd1c11` | `…/tiers/warlord/Orc_High_Warchief_Mascot_Warlord` | false | passed(owner, 2026-06-29) | `236f7f7e9509125a749371e7375f7c7d64dceecc5f8ef7931dab408bbd1ffef9` |
    - ▼ **애니메이션 기록 — `orc-high-warchief-mascot` T1(veteran) `e1e3b502-…`(2026-06-30, 8-frame v3)**. 표준·prompt는 [[15-Character-State-Model]] §4.4, 런타임 소비는 [[SPEC-302-mascot-prestige-tiers]] §3.4. mode=v3, frame_count=8(방향당 9파일 = reference 0 + animated 1–8), 비용 56 gen/state(7/dir×8). **6 state 41 clip × 9 frame = 369 PNG** → `…/tiers/veteran/Orc_High_Warchief_Mascot_Veteran/animations/<state>/<dir>/frame_%03d.png`, manifest `prestige.tiers[veteran].animations` 연결 완료(검증: 369/369 파일 존재, web 218 tests green).
      - state별: idle(8dir, fps4)·roaming(8dir, fps8)·active(8dir, fps8)·waiting(8dir, fps4)·stale(8dir, fps3)·error(south-only, fps6). roaming(west)는 1차 누락분 1 clip 보충 생성.
      - **active = ember 이펙트 강화(사용자 지시)**: 도끼 스윙에 ember fire arc 트레일 + 상승 spark(전·후면 8방향에서 무기 grip 유지·이펙트 노출 확인). 초기 plain active는 삭제 후 이펙트 prompt로 재생성.
      - **rollout([[SPEC-302-mascot-prestige-tiers]] §3.6, 2026-06-30)**: 도입 순서 = **모든 character T1 먼저 → 이후 각 character T2·T3**. mascot T1 = `available`(rotation+animation 완비). **mascot T2·T3 = `staged`, 다른 4 character의 T1 생성 이후 진행.** **T2(champion)는 망토(cape)가 T1 대비 단조로워 rotation만 재생성**(animation 미진행) — 신규 char id `55edad33-8b53-4a4d-a2af-0a9e75f503ee`로 **재생성·승인 완료**(crimson fur-trim ornate mantle로 개선, axe 8방향 grip 일관·single·text 없음). 구 `a27bf339` 삭제, manifest `pixellab_character_id`·SHA 갱신 완료. champion animation은 **다른 4 character T1 생성 이후** 진행.
    - ▼ **다른 4 character T1~T3 rotation 생성 기록(2026-06-30, animation 미생성·전 tier `staged`)**. "모든 character T1 먼저" 도입을 위해 4종 × 3 tier rotation을 `create_character_state`(각 base에서 직접)로 생성, manifest `prestige` 블록 추가(status `staged`; owner 직접 검수 후 T1→`available`). 무기/IP 일관성 규칙 적용. SHA = 8 rotation(S,E,N,W,SE,NE,NW,SW) concat sha256.
      - **검수 이슈·정정**: ① storm-shaman T2 일부 전면 방향 hood 벗겨짐 → hood-up clause 재생성. ② codex T2 1차 content-policy 차단 → 표현 완화 재생성. ③ **unknown 전면 개정**(fel demon-hunter, archetype ref=Illidan-class): single→**쌍검 dual-wield 양손·8방향 둘 다**, T3 **악마 날개** 추가 — IP 발산(곡선 warglaive·blindfold 금지, 직선 original blade). storm-shaman T3 / unknown T2·T3 / iron-commander T2·T3 = **blocking IP 리뷰 대상**(§2.7).
      | character | tier | suffix | `pixellab_character_id` | status | IP | SHA8 |
      | --- | --- | --- | --- | --- | --- | --- |
      | storm-shaman | 1 | `-adept` | `50882ad6-…` | staged | n/a | `81813daf` |
      | storm-shaman | 2 | `-tempest` | `b4a58ffe-…` | staged | n/a (hood-fix) | `65445ae5` |
      | storm-shaman | 3 | `-archon` | `573066bf-…` | staged | blocking | `63234cd7` |
      | codex | 1 | `-senior` | `baf019e9-…` | staged | n/a | `8687b447` |
      | codex | 2 | `-artificer` | `6b86fb49-…` | staged | n/a (policy 재생성) | `5eda65c8` |
      | codex | 3 | `-forgewright` | `18e1005f-…` | staged | n/a | `3082f7db` |
      | unknown | 1 | `-feltouched` | `84692d27-…` | staged | n/a | `206026c2` |
      | unknown | 2 | `-felreaver` | `50828e10-…` | staged | blocking | `2bce72a9` |
      | unknown | 3 | `-felascendant` | `740ad695-…` | staged | blocking | `0ff7e0f1` |
      | iron-commander | 1 | `-enforcer` | `94c3142f-…` | staged | n/a | `966f9914` |
      | iron-commander | 2 | `-marshal` | `7c2eacea-…` | staged | blocking | `3cea3296` |
      | iron-commander | 3 | `-sovereign` | `7ca301a7-…` | staged | blocking | `3a6da801` |
      - ▼ **T1 애니메이션 기록(2026-06-30, 8-frame v3)**: "모든 character T1 먼저" 정책으로 storm-shaman·codex·unknown·iron-commander **T1**에 핵심 3상태 **idle·active·roaming**(8방향, 방향당 9프레임) 생성·pack 반영·manifest `prestige.tiers[T1].animations` 연결 완료. waiting·stale·error는 예산상 미생성 → SPEC-302 `static_tier` 폴백. active는 T1-modest 테마 연출(shaman teal-storm·codex tech·unknown fel·iron red-iron). download zip(`/mcp/characters/<id>/download`, Bearer)로 추출. **1프레임 결손 2건 보정**(unknown idle/south frame_001, iron idle/north frame_008 — 생성 누락분을 인접 프레임 복제로 패딩; 미세 idle 루프라 무영향). **잔여 generation 69**(구독 소진 근접) — T2/T3·나머지 상태 animation은 충전/리셋 후. 전 tier `staged` 유지(owner 검수 후 T1→available).
      - ▼ **T1 나머지 상태 애니메이션 기록(2026-07-27, 8-frame v3, Phase 2 착수)**: 구독 리셋 확인(get_balance: 5000/5000, Tier 2 Pixel Artisan) 후 storm-shaman(`-adept`)·codex(`-senior`)·unknown(`-feltouched`)·iron-commander(`-enforcer`) **T1**의 남은 3상태 **waiting·stale·error**(8방향, 방향당 9프레임, fps는 manifest 표준 waiting4/stale3/error6)를 생성·pack 반영·manifest `prestige.tiers[T1].animations` 연결 완료. mascot T1과 동일하게 이번 4종도 error를 south-only가 아닌 **8방향 전부**로 생성(base tier가 이미 8방향이었던 것과 정합). action_description은 §4.4 mascot 템플릿을 캐릭터별 장비로 치환(shaman: carved staff / codex: terminal tablet·tool belt / unknown: dual fel-touched swords, 둘 다 grip·노출 명시 / iron-commander: command warhammer). download zip으로 추출 시 codex `waiting`(south-west)이 최초 시도에서 `Generation failed`로 누락 → 동일 `animation_group_id`에 재요청해 8/8 보정. **QA 메모**: unknown `stale`(west) 일부 중간 프레임에서 두 blade가 겹쳐 한 자루만 또렷이 보이는 경미한 각도 이슈 확인(reference frame·대부분 프레임은 양손 grip 정상 노출) — IP 규칙(두 자루 모두 매 방향 노출)의 핵심 위반은 아니라고 판단해 재생성 없이 채택; 향후 QA에서 재확인 권장. **T1 = 5종 전 캐릭터 6-state(idle/roaming/active/waiting/stale/error) 완비**(terminated는 전 캐릭터 공통 `coverage:"none"` static fallback, 생성 대상 아님). storm-shaman T3(archon)·unknown T2/T3(felreaver/felascendant)·iron-commander T2/T3(marshal/sovereign)는 §2.7 blocking IP 리뷰 대상이라 이번 범위에서 제외. web 테스트 412/412 통과 확인.
      - ▼ **T2·T3 애니메이션 기록(2026-07-28, 8-frame v3, Phase 2 완료)**: §2.7 blocking IP 리뷰를 **통과한** 5개 tier — `orc-high-warchief-mascot` T2(`-champion`)·T3(`-warlord`), `orc-claude-storm-shaman` T2(`-tempest`), `orc-codex-field-engineer` T2(`-artificer`)·T3(`-forgewright`) — 전부 6-state(**idle·roaming·active·waiting·stale·error**, 8방향, 방향당 9프레임, fps는 manifest 표준)를 생성·pack 반영·manifest `prestige.tiers[T2/T3].animations` 연결 + `status: "staged"→"available"` 승격 완료. T3 active는 §4.4 정책대로 캐릭터 테마 aura를 극단적으로 강화(mascot ember-storm / shaman teal-storm-lightning / codex tech-forge aura+부유 tool). action_description은 §4.1/§4.4 무기(단일) grip 유지 규칙 적용. download zip 추출 중 개별 direction 재시도 2건(codex T2 active south-west, codex T3 waiting south 등) 발생 — 동일 `animation_group_id`로 재요청해 8/8 보정. codex T2(artificer) 압축 해제 시 `active` 폴더명이 action_description 파생 긴 이름으로 나와 `active`로 명시 rename 후 복사. QA 샘플(active/idle/error 대표 방향) 육안 확인 — 무기·장비 일관성, text artifact 없음, tier간 실루엣 구분 확인. **§2.7 blocking IP 리뷰 미통과 5개 tier(storm-shaman T3 archon, unknown T2/T3 felreaver·felascendant, iron-commander T2/T3 marshal·sovereign)는 owner 승인 전까지 이번 범위에서 제외, `staged` 유지.**
      - ▼ **T2·T3 애니메이션 기록(2026-07-29, 8-frame v3, Phase 2b — owner IP 승인 후 완료)**: 사용자(owner)가 §2.7 blocking 게이트 대상 5개 tier의 IP를 명시적으로 승인 → `orc-claude-storm-shaman` T3(`-archon`), `orc-unknown` T2(`-felreaver`)·T3(`-felascendant`), `orc-iron-commander` T2(`-marshal`)·T3(`-sovereign`) 전부 6-state(**idle·roaming·active·waiting·stale·error**, 8방향, 방향당 9프레임, fps는 manifest 표준)를 생성·pack 반영·manifest `prestige.tiers[T2/T3].animations` 연결 + `status: "staged"→"available"` 승격 완료. action_description은 §4.1/§4.4 규칙 그대로 적용: shaman(archon)=great runed storm staff, unknown(felreaver/felascendant)=dual fel combat swords(**"BOTH blades gripped, visible in every direction including rear views"** 문구 매 state 유지), iron-commander(marshal/sovereign)=command war hammer. T3 active는 캐릭터 테마 aura 극단 강화(shaman teal-gold storm+lightning halo / unknown fel-fire aura+대형 날개+ember / iron-commander red-iron shockwave+iron rune halo). **재시도 사례**: PixelLab 서버 부하로 여러 direction이 반복적으로 ~900s ETA 백로그에 걸림(특히 iron-commander T3 roaming·waiting·active·stale) — 동일 `animation_group_id`로 대기 후 자동 완료, 8/8 확보. iron-commander T3 `stale`은 north 1방향이 그룹에서 완전히 누락되어 별도 1-direction 재요청(7 gen)으로 보정. **QA**: unknown felreaver/felascendant 모두 정면·후면에서 양손 grip 확인, west/east 순수 profile 각도에서 한쪽 팔 자연 occlusion 발생(Phase 2·Phase 1과 동일 패턴, 핵심 규칙 위반 아님으로 판단해 재생성 없이 채택). unknown felascendant는 눈가리개 없음·직선 blade(곡선 warglaive 아님)·original 뿔/날개 형태 확인. iron-commander sovereign은 실루엣·투구·hammer head가 특정 기존 캐릭터와 구분됨을 육안 확인. **§2.7 blocking 게이트 대상 5개 tier 전부 owner 승인·생성·반영 완료 — 5종 character 전 tier(T0~T3, 20 variant) 6-state animation 완비.** web 테스트 412/412 통과 확인.

11. **Epic monster NPC (background별 6 variant, 512×512) — planned, 미생성**: camp scene마다 환경에 맞는 거대 boss 몬스터를 ambient NPC로 추가(scene당 1마리, 비-상호작용, `ground.polygon` 전체 roaming). 6 variant ↔ 6 background: `monster-mosshide-behemoth`(orccamp-default)·`monster-frostfang-colossus`(froststeel)·`monster-magma-colossus`(emberforge)·`monster-bog-leviathan`(mirebog, 배경 등록·gate PASS 0.3274)·`monster-duneplate-scourge`(sunscorch, 테마 미생성)·`monster-bonewraith-revenant`(necropolis). animation 5종 `active`/`waiting`/`idle`/`roaming`/`error`(roaming 8방향, dwell south MVP; stale/terminated 없음). 설계·prompt·manifest schema는 [[16-Epic-Monster-NPC]], background별 art 컨셉은 [[background-tile-merge-guide]] §6, 런타임 행동은 [[SPEC-303-epic-monster-npc]], 렌더는 [[SPEC-300-asset-rendering]] §2.7/§3.10. **생성 경로(2026-06-29 갱신)**: PixelLab MCP 인증 **확인됨**(구독 active). 단 **512는 MCP로 불가**(`create_character` size≤128 → export ~247px)라 **PixelLab 웹 UI에서 512 생성 후 export zip을 import**한다(사용자 결정). asset pack v0.2.0에서 variant별 `pixellab_character_id`·export zip + SHA-256·extracted root `sprites/monsters/<key>/`·사용 prompt·seed·8방향 QA를 본 섹션에 기록 → manifest `monsters` 블록 `status:"available"` + `backgrounds.<bg>.epic_monster` 링크 실값 반영.
    - 진행 시 채울 항목(variant별): `pixellab_character_id`, `body_type`, export zip + SHA-256, extracted root, frame_size(실 export 값)·anchor(발 접지)·seed, `roaming` 8방향/dwell south 완료 + 실루엣 구분·IP 오인 QA.
    - **DELIVERED (2026-06-30) — `monster-mosshide-behemoth` (Mosshide Behemoth, orccamp-default)**: pixellab_character_id `5975f4f2-8733-4980-bcd1-002179d15d61`, body_type `quadruped`(bear template) **mode `pro`**(40 gen) base. **frame_size 256×256**(MCP `create_character` size 128 → export 256; 512 native는 MCP 불가라 256 채택 + 런타임 render-scale로 epic — [[16-Epic-Monster-NPC]] §2), anchor `[128,186]`(발 접지, south rotation trim 측정). **animation 5종 모두 9프레임 v3 custom**(`animate_character` mode v3 `frame_count=8`; quadruped도 v3 custom 동작 확인): `roaming` 8방향 + `idle`/`active`/`waiting`/`error` south. extracted root `asset-packs/orc-camp-default/sprites/monsters/monster-mosshide-behemoth/`(rotations 8 + animations/<state>/<dir>/frame_%03d.png), download zip SHA-256 `a897f864116be0a7af280c2fd8363bc66741e6deb4417ec932d58b56f8694672`(2026-06-30 갱신 — **`roaming` east 방향이 반대로 생성되어 mirror로 재생성**해 9프레임 교체 적용; 그 외 방향/상태는 동일. 이전 SHA `fe3ea8e6…`). manifest `monsters.monster-mosshide-behemoth.status:"available"` + `backgrounds.orccamp-default.epic_monster` 링크 반영. (아래 5종도 동일 파이프라인으로 인도 완료 — **6/6 DELIVERED**, web 테스트 232/232 통과.)
    - **DELIVERED (2026-06-30) — `monster-frostfang-colossus` (Frostfang Colossus, froststeel-camp)**: id `48a4e03f-0aef-431f-bbda-742f999d7cb3`, quadruped(bear) mode `pro`, frame **252×252**, anchor `[126,181]`, 5종 9프레임 v3 custom(roaming 8dir + idle/active/waiting/error south), root `sprites/monsters/monster-frostfang-colossus/`, zip SHA-256 `3bc3d03a48c5b5117c72d0594b8f140fc04b78a776b5a421158e10f7840b463d`. manifest available + `backgrounds.froststeel-camp.epic_monster` 링크.
    - **DELIVERED (2026-06-30) — `monster-magma-colossus` (Magma Colossus, emberforge-camp)**: id `b8423581-9602-4ad5-ba5b-93ef5bb17f3e`, quadruped(bear) mode `pro`, frame **244×244**, anchor `[122,175]`, 5종 9프레임 v3 custom, root `sprites/monsters/monster-magma-colossus/`, zip SHA-256 `c71f56517f004b3664fb1e600615a3106ab044b138fc6d3ead82ae27ae43e096`. manifest available + `backgrounds.emberforge-camp.epic_monster` 링크.
    - **DELIVERED (2026-06-30) — `monster-bog-leviathan` (Bog Leviathan, mirebog-camp)**: id `f5b0a130-cf5d-4c25-a948-dc967f3bc1e3`, quadruped(bear) mode `pro`, frame **252×252**, anchor `[126,180]`, 5종 9프레임 v3 custom, root `sprites/monsters/monster-bog-leviathan/`, zip SHA-256 `59181faac5f67a33068fbc4e396c81a0d24095e858bab5253cb4780cdf68b4e8`. manifest available + `backgrounds.mirebog-camp.epic_monster` 링크.
    - **DELIVERED (2026-06-30) — `monster-duneplate-scourge` (Duneplate Scourge, sunscorch-camp)**: id `7a6071fb-d0e8-4073-8955-e35dab19f680`, quadruped(lion) mode `pro`, frame **256×256**, anchor `[128,184]`, 5종 9프레임 v3 custom, root `sprites/monsters/monster-duneplate-scourge/`, zip SHA-256 `e9f8850f8632e3cbfad9e624343370005e8b0d962e67045d0b2f6d2fb14cff82`. manifest available. **`sunscorch-camp` 배경 미등록**이라 `backgrounds.*.epic_monster` 정방향 링크는 없음(`monsters.*.background`만; 해당 배경 생성 시 링크 추가).
    - **DELIVERED (2026-06-30) — `monster-bonewraith-revenant` (Bonewraith Revenant, necropolis-camp)**: id `2197cb44-86c6-411a-9427-3b7ce9145f9e`, **humanoid** mode `v3`, frame **248×248**, anchor `[124,185]`, 5종 9프레임 v3 custom, root `sprites/monsters/monster-bonewraith-revenant/`, zip SHA-256 `b8763663d26b1c67f02a7d3ff17e54497e6e004fa30e915f92ebbd18747180d6`. manifest available + `backgrounds.necropolis-camp.epic_monster` 링크.
    - **요약**: 6/6 모두 `status:"available"`. 크기 244–256px(MCP `create_character` size 128 cap → export ~2×; 512 native는 MCP 불가, 런타임 render-scale로 epic). animation은 전부 **9프레임 v3 custom**(`animate_character` mode v3 frame_count 8; quadruped도 v3 custom 동작). `sunscorch-camp` 배경만 미등록이라 그 variant는 bg 정방향 링크 보류. 총 sprite 파일 696개(6 × 116).

12. **Character avatar portraits (BG-style 2:3 bust) — 20종(5 base + 15 tier) delivered (2026-06-30, owner 외부 제작)**: 5종 character의 Baldur's Gate 풍 정면 흉상(bust) portrait(머리→가슴, 정적, 2:3 slot, 장식 frame은 web CSS 소유) + 각 character의 prestige tier(T1~T3) portrait. 설계·prompt·runbook·manifest `portraits` schema는 [[17-Character-Avatar-Portraits]], 런타임 렌더/배치/수용 기준은 [[SPEC-304-character-avatar-portraits]].
    - **생성 경로**: PixelLab `create_map_object`(view=side) probe로 실현가능성을 확인한 뒤, **owner가 외부 image-gen으로 고품질 pixel 흉상 20종을 직접 제작·제공**(`~/Downloads/portrait-*.png`, 1254×1254)해 채택 — 초기 PixelLab probe 산출물(280×400)은 **대체·폐기**. 채택 시 `magick`으로 **512×512 다운스케일 + lossless WebP** 변환(`portraits/` 4.4MB).
    - **편차(의도적·수용, [[SPEC-304-character-avatar-portraits]] §6)**: (a) **baked dark 배경**(투명 아님; BG framed 초상과 정합, CSS frame이 위를 감쌈; 장식 frame은 미-baked). (b) **512×512(1:1)** source; 2:3 slot이 `object-fit:cover`로 좌우 약간 crop. 실측 source = manifest `source_size`=[512,512]·`frame_aspect`=`2:3`가 SSOT.
    - **tier 매핑**: owner 파일명 `-t-1/-t-2/-t-3` → SPEC-302/doc15 named suffix(아래). tier는 owner 직접 제작이라 **IP 리뷰 충족**(종전 high-tier blocking 해소).
    - **delivered 기록**(파일 `portraits/<key>.webp`, tier `portraits/tiers/<suffix>/<key>-<suffix>.webp`, lossless WebP 512×512):
      | character key | tier(suffix) | SHA-256 |
      | --- | --- | --- |
      | `orc-high-warchief-mascot` | base | `e369132132357fdd81b7f328314cb3972998758d21f3e2fc936eae615ba83f8b` |
      | `orc-high-warchief-mascot` | T1 veteran | `403fe78f00fdf3b72c9d676da7cb7879beb6d666c01b611409df65edcb2d661b` |
      | `orc-high-warchief-mascot` | T2 champion | `31df15ced9c7a02c593f3f39040eac6a17c6cdfe629d6376b285bbf264484596` |
      | `orc-high-warchief-mascot` | T3 warlord | `f4f202ec1bccff62c689de9a5e84bb8052566dfc30a81a615b618e526f4eaf3f` |
      | `orc-claude-storm-shaman` | base | `94a01c996fc9449271bd2d69601f8e8bd412834ebce41cb2563b4bc5b0f37e40` |
      | `orc-claude-storm-shaman` | T1 adept | `01a5e47465bbb099fc8e73c96c00cd8f1ce6e6c41eefa82542e3b8ca2f174267` |
      | `orc-claude-storm-shaman` | T2 tempest | `5f9ccb6a92b1f7d4690ea1fd5908268da559a5c736c976ae34038a9d652b584c` |
      | `orc-claude-storm-shaman` | T3 archon | `504919412dbb96dee9681eb730612cdfe1301c739d606b46b8bad976ba79f71a` |
      | `orc-codex-field-engineer` | base | `3939bdd607e0d32026acede10a5ae925e150c11b6edaa9f280391ea403d484de` |
      | `orc-codex-field-engineer` | T1 senior | `fac140f85cf0d87b5238087e4c60a131cd44c65bfaa250d3969c6208d4b56bd9` |
      | `orc-codex-field-engineer` | T2 artificer | `9691846016548121fae08e68385b2a2e289cda33baa45f68762051b9bb7f6fbe` |
      | `orc-codex-field-engineer` | T3 forgewright | `c941f9ac5aab03b1d4d3aea87b1c9a612551f4a6a49a2c3876b77e8338ab72b0` |
      | `orc-unknown` | base | `2bb341523c60f0620ae05b05074b3478bfb015f1789b1b8ad28e066124d5462c` |
      | `orc-unknown` | T1 seasoned | `0fdfa4f53e454979c39328757b36842b284b3b206bba4b851717f60967653fea` |
      | `orc-unknown` | T2 veteran | `929fa08942e6c3f7c23ac90b6c9dc39eac84ef83d82fab894f8ef60cd8d238e1` |
      | `orc-unknown` | T3 elder | `fc32bae85f917c63a965023180a2579c9b9927e3a4bf9a3f0f5c5eb4b5eacaf6` |
      | `orc-iron-commander` | base | `85af40ff581a1ecb43e7dc75f789600f57a4020fcd17afe8586f39530f0e53e3` |
      | `orc-iron-commander` | T1 enforcer | `077e21138d21a174de4a5631fd511128e6d3230f2f4461b5186f7a02080e3bc9` |
      | `orc-iron-commander` | T2 marshal | `460c7220155aa62df3d5b93ec71c4d8cde5d29de6c7dcb41e0d9fd9805ed3d7d` |
      | `orc-iron-commander` | T3 sovereign | `16eacdfa371049cbd0145d8207d0a29c1dbee2c816078f66a9345932d70bc62b` |
    - **manifest**: `portraits` 블록(v0.2.0, root `portraits`, frame_aspect `2:3`, source_size [512,512])에 5 base + 각 character `tiers` 맵 반영. 런타임 resolve·표시·tier 매핑 검증 `web/tests/portraitManifest.test.ts`·`prestige.test.ts`(257 tests green).
    - **tier 소비 seam**: `web/src/assets/prestige.ts` `displayedTierForOrc(orc)`(현재 0=base, `Orc.usage` 미수집으로 SPEC-302 forward) → `OrcInspector` → `resolvePortrait`(displayedTier→tier portrait). usage 데이터 도입 시 자동 tier 표시.
    - **license**: 게이트(item 3) 상속 — 외부 제작물이므로 PixelLab 약관과 무관하나, 배포 license는 owner 확인 대상.
